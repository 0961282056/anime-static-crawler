...
目前最重要的工作不再是重寫架構。Actions 預設權限、Repository-level 重複 secrets、Discord 去敏、Dependabot、瀏覽器 smoke、retention CLI coverage、圖片像素限制與 workflow 靜態分析都已完成；接下來以 schema format、Windows directory retry、災難復原量化與 CSP 收緊為主。
...
2026-07-14 本機完整驗證：159 tests 全部通過、branch coverage 78.48%、36 季／2,067 筆／9 年資料通過，3 個 static assets 完全一致；live selector canary 另成功解析 2026 夏季 75 張卡片。PR 合併前仍以 GitHub `quality` required check 作為正式 gate。
...
目前正式門檻已足以阻擋大部分資料破壞；仍可補強 schema 欄位格式、瀏覽器無障礙檢查、CSP 收緊後的前端回歸，以及 Windows static directory swap retry。
...
- 本次已用本機 `.venv` 的 Python 3.11.9 重新執行 159 項測試：全部通過，branch coverage 78.48%，高於 70% 門檻；BUILD_ONLY 驗證 36 個季度、2,067 筆、9 年資料與 3 個靜態資產完全一致；live selector canary 成功解析 2026 夏季 75 張卡片。
...
目前不需要再次大改核心架構。下一步依序完成 schema format、Windows directory retry、災難復原 RTO／RPO、squash-only／自動刪分支與 CSP 收緊；這些工作比現在導入資料庫更直接地提升可靠性。
