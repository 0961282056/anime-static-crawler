...
目前最重要的工作不再是重寫架構。Actions 預設權限、Repository-level 重複 secrets、Discord 去敏、Dependabot、瀏覽器 smoke、retention CLI coverage、圖片像素限制、schema format、Windows directory retry、災難復原目標與 workflow 靜態分析都已完成；接下來以備份／restore 演練與 CSP 收緊為主。
...
Alpine 已改成本站託管、固定 SHA-256 的 `@alpinejs/csp` build，`script-src` 已移除 `'unsafe-eval'`。目前 CSP 仍允許 `style-src 'unsafe-inline'`，因分享圖片功能與既有模板仍會建立 inline style；需先完成樣式重構與瀏覽器回歸才能移除。其餘外部 CDN 仍有版本鎖與 SRI，之後可逐步自架。
...
- Repository 只允許 squash merge；merge commit 與 rebase merge 已停用。
- Repository 已開啟 auto-merge，PR 合併後會自動刪除來源分支。
...
目前正式門檻已足以阻擋大部分資料破壞；仍可補強瀏覽器無障礙檢查與 CSP 收緊後的前端回歸。
...
7. [完成] Windows static directory swap 加有限次 retry／backoff；重試耗盡仍保留舊輸出並 fail closed。

### P3：長期最佳化

1. [第一階段完成] Alpine 已改用本站託管、固定雜湊的 CSP build，`script-src` 已移除 `unsafe-eval`；分享圖片仍依賴動態 inline style，因此 `style-src 'unsafe-inline'` 要在樣式重構與 Playwright 回歸後才移除。
2. [完成] Repository 只允許 squash，並開啟合併後自動刪分支。
3. Flat JSON cache 目前 2,279 entries 很合適；只有成長到數十萬筆、競爭寫入或查詢變複雜時，才考慮 SQLite／object storage index，不必現在為了架構感而導入資料庫。
4. [完成文件與目標] 災難復原手冊已補 RTO／RPO、責任人與異地 Git／JSON 備份週期；Cloudinary restore 尚無完成紀錄，正式 retention execute 前必須先演練，之後每季一次。
...
- 本次已用本機隔離環境的 Python 3.12.13 重新執行 188 項測試：全部通過，branch coverage 78.98%，高於 70% 門檻；BUILD_ONLY 驗證 36 個季度、2,067 筆、9 年資料與 5 個靜態資產完全一致；正式 `quality` 仍以專案指定的 Python 3.11 重驗。live selector canary 已成功解析 2026 夏季 75 張卡片。
...
目前不需要再次大改核心架構。下一步依序完成第一份異地備份與 restore 演練，再進行 CSP 收緊；這些工作比現在導入資料庫更直接地提升可靠性。
