...
目前最重要的工作不再是重寫架構。Actions 預設權限、Repository-level 重複 secrets、Discord 去敏、Dependabot、瀏覽器 smoke、retention CLI coverage、圖片像素限制、schema format、Windows directory retry 與 workflow 靜態分析都已完成；接下來以災難復原量化與 CSP 收緊為主。
...
目前不需要再次大改核心架構。下一步依序完成災難復原 RTO／RPO、squash-only／自動刪分支與 CSP 收緊；這些工作比現在導入資料庫更直接地提升可靠性。
