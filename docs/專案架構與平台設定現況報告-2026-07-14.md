...
> 修改基準：`origin/main` commit `751b2ae`（`docs: refresh project operations documentation (#4)`）
...
| 本機開發環境 | 綠 | Python 3.11.9／144 tests 正常；已同步 `main` 並從 `751b2ae` 建立新分支 |

## 2. 報告依據與限制

本報告交叉使用下列證據：

- 正式 Git：本機 `main` 已 fast-forward 到 `origin/main` commit `751b2ae`，本次監控修改分支由該 revision 建立。
...
3. 最後刪除上述 Repository secret 名稱。
4. 再跑一次人工 crawler，確認 Discord、Cloudinary 與自動 PR 都正常。
...
2026-07-14 本機完整驗證：144 tests 全部通過、branch coverage 79.25%、36 季／2,067 筆／9 年資料通過，移除 Sentry 後 2 個 static assets 完全一致。PR 合併前仍以 GitHub `quality` required check 作為正式 gate。
...
本機 `main` 已 fast-forward 到 `origin/main` commit `751b2ae`，目前工作位於由該 revision 建立的 `codex/discord-only-monitoring`。現有 `.venv` 已用 Python 3.11.9 成功執行完整測試，可繼續使用。
...
- 本次已用本機 `.venv` 的 Python 3.11.9 重新執行 144 項測試：全部通過，branch coverage 79.25%，高於 70% 門檻；BUILD_ONLY 驗證 36 個季度、2,067 筆、9 年資料與 2 個靜態資產完全一致。
...
1. 撤銷／移除 Repository-level 重複 secrets。
2. 開啟 Dependabot security updates。
