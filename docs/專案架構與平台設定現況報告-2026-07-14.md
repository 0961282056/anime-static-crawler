...
> 正式程式基準：`origin/main` commit `093137a`（`chore(data): update anime data (#3)`）
...
目前最重要的工作不再是重寫架構，而是完成平台層收尾：

1. GitHub Actions 平台預設 `GITHUB_TOKEN` 仍是 `write`，與文件要求的唯讀不一致。
2. Repository-level 仍保留 7 個舊／重複 secrets，權限範圍比 Environment secrets 更廣。
3. Discord exception 去敏、Dependabot 與瀏覽器 E2E 等程式／品質工作仍待後續 PR。
...
| Secret 最小權限 | 黃／紅 | Environment 分層正確，但 Repository secrets 尚未清除 |
...
| Sentry | 黃 | 程式支援，但目前有效狀態不清楚；Repository secret 仍可能被 crawler 讀取 |
| 前端測試 | 黃 | 有語法與靜態建置 gate，缺瀏覽器 E2E／無障礙自動測試 |
| 本機開發環境 | 綠 | Python 3.11.9／143 tests 正常；已同步 `main` 並從 `093137a` 建立新分支 |

## 2. 報告依據與限制

本報告交叉使用下列證據：

- 正式 Git：本機 `main` 已 fast-forward 到 `origin/main` commit `093137a`，文件工作分支由該 revision 建立。
...
| `static/` | CSS、main.js、monitoring.js 唯一來源 | `static/css`、`static/js` |
| `dist/data/` | Git 追蹤的正式季度資料 | 36 個 JSON |
| `build.sh` | Cloudflare 唯一正式建置入口 | 全檔 |
| `_headers` | Cloudflare CSP、安全標頭與快取規則 | 全檔 |
| `.github/workflows/` | Quality Gate、crawler、retention plan/execute | 4 個 workflow |
| `tests/` | parser、schema、error、retention、workflow safety 等測試 | 12 個測試模組 |
...
| Sentry SDK | 2.64.0 | 選用的 Python 錯誤監控 |
...
`requirements-build.txt`、`requirements.txt`、`requirements-dev.txt` 都是完整版本與 hash 鎖定。直接依賴寫在相對應的 `*.in`，正式安裝一律使用 `--require-hashes` 的 `*.txt`。
...
| Sentry Browser | 7.114.0 | 有 Browser DSN 時才載入 |
...
實際 YAML 已比平台預設更嚴格：各 workflow 明確宣告最小 `permissions`，第三方 action 也全部鎖定完整 commit SHA。換句話說，目前程式碼層面安全，但平台仍留下「將來新增 workflow 時可能忘記收緊」的空間。

建議把 Repository 的 Default workflow permissions 改成 **Read repository contents permission**。若 GitHub 方案／介面提供限制 actions 或要求完整 SHA，也可開啟；在此之前保留 CI 中的 SHA 檢查。
...
其中最實用的下一步是開啟 Dependabot security updates，讓 lock 檔中的依賴出現已知漏洞時自動提出 PR；CI 的 `pip-audit` 仍繼續作為第二道防線。

### 9.6 Repository-level secrets 清理

Repository 層仍能看到下列 secret 名稱：
...
目前 workflow 已改用 Environment；Cloudflare token／account 也未被現行程式或 workflow 引用。這些 Repository secrets 的作用域過廣，而且與 Environment 重複。

安全清理順序不是直接按刪除，而是：

1. 先確認 Environment 中的新值可正常跑完 crawler。
2. 對外部平台撤銷不再使用的 Cloudflare token、舊 Cloudinary key、舊 Discord webhook。
3. 決定是否保留 Sentry；若保留，改放 `crawler-production`，若不保留則在 Sentry 撤銷。
4. 最後刪除上述 Repository secret 名稱。
5. 再跑一次人工 crawler，確認 Discord、Cloudinary 與自動 PR 都正常。

注意：GitHub Environment secret 不會保證自動遮蔽同名 Repository secret 的所有未來用法；把不用的高權限秘密真正撤銷，才是完整收尾。
...
2026-07-13 的 main 正式 CI 證據：143 tests 全部通過、branch coverage 78.97%、`pip-audit` 找不到已知漏洞、36 季／2,067 筆／9 年資料通過，3 個 static assets 完全一致。
...
### 10.3 Cloudinary Retention Dry Run
...
### 10.4 Cloudinary Retention Execute
...
## 12. Cloudinary、Discord 與 Sentry
...
仍有一個程式層安全缺口：`services/notifier.py` 目前會把 requests exception 原文放進 `NotificationError`；HTTP error 可能包含完整 webhook URL，外層又會輸出到 stderr。GitHub masking 通常會遮蔽 secret，但不應依賴 masking 作為唯一防線。應改為只回報 exception 類型／安全狀態碼、不帶 URL，並補測試確認 log 中永遠沒有 webhook。

### 12.3 Sentry

- 前端只有設定 `SENTRY_BROWSER_DSN` 時才載入 Sentry；目前 Cloudflare 截圖未看到這個變數，所以瀏覽器 Sentry 看起來未啟用。
- 即使未來啟用，`monitoring.js` 的 Replay session／error sample rate 都是 0，不會錄製使用者畫面。
- Python crawler 會讀 `SENTRY_DSN`。`crawler-production` 沒有這個 secret，但 Repository-level 仍存在同名 secret，所以目前不能只靠 Environment 畫面斷言後端 Sentry 一定關閉。

建議做一個明確決策：要用就把有效 DSN 限縮到 `crawler-production` 並刪除 Repository copy；不用就從 Sentry 撤銷 DSN 並刪除 GitHub secret。不要維持「不知道是否仍生效」的中間狀態。
...
目前正式門檻已足以阻擋大部分資料破壞，但還缺：

1. **瀏覽器 E2E**：沒有 Playwright 類測試真正載入首頁、切年份／季節、fetch JSON、檢查 console 與 404。
2. **不可逆 CLI coverage**：`pyproject.toml` 把 `cloudinary_cleaner.py` 排除 coverage；其 helper 有大量測試，但 739 行 CLI orchestration 不受 70% 門檻約束。
3. **通知去敏回歸測試**：現有測試只驗證 raise，沒驗證 exception／log 不得含 webhook URL。
4. **selector canary**：fixture 能測已知 HTML，但沒有每日只讀 canary 提早警告來源 DOM 改版。
5. **圖片像素上限**：有 10 MiB byte limit，尚未對 width × height 設上限。
6. **Workflow 靜態分析**：長篇 Bash guard 有測試保護，但 CI 尚未加入 `actionlint`／ShellCheck。
...
1. 把 Actions default workflow permission 改成 Read。
2. 按「先撤銷外部憑證、再刪 GitHub secret」順序移除 Repository-level 重複秘密。
3. 明確決定 Sentry 保留或停用。
4. 修正 Discord exception 去敏並補洩漏測試。
5. 開啟 Dependabot security updates。

### P2：品質與可維護性

1. 增加 Playwright smoke test：首頁可載入、資料 request 成功、主要篩選可操作、console 乾淨、圖片無破損。
2. 讓 `cloudinary_cleaner.py` 的核心 orchestration 納入 coverage／獨立門檻。
3. 將系統錯誤與單筆 parse 錯誤分型；即使日後放寬 parse failure ratio，系統錯誤仍永遠阻擋寫入。
4. 強化 schema format：`bangumi_id` pattern、星期列舉、`HH:MM`、來源 URL 與 Cloudinary cloud/path。
5. 新增來源 selector canary、圖片 pixels limit。
6. Windows static directory swap 加有限次 retry／backoff，降低開啟檔案造成的 WinError 5。
...
flowchart LR
    ...
    C --> O["Discord crawler summary / optional Sentry"]
    Q --> O
...
- **觀測面**：Discord 做日常摘要；Sentry 必須明確選擇；資料品質與 browser smoke 提早發現退化。
...
本機 `main` 已 fast-forward 到 `origin/main` commit `093137a`，目前文件工作位於由該 revision 建立的 `codex/docs-platform-hardening`。現有 `.venv` 已用 Python 3.11.9 成功執行完整測試，可繼續使用。
...
- 本次已用本機 `.venv` 的 Python 3.11.9 重新執行 143 項測試：全部通過，branch coverage 78.97%，高於 70% 門檻。測試停用 pytest cache／pyc，coverage 寫入系統暫存區。本次只新增文件，沒有修改執行程式。
...
目前不需要再次大改核心架構。應先完成三項平台收尾：

1. 收緊 Actions default token 並撤銷／移除 Repository-level 重複 secrets。
2. 明確決定 Sentry 保留或停用，並開啟 Dependabot security updates。
3. 修正 Discord 錯誤訊息去敏。

完成這三項後，再依 P2 補 browser E2E、不可逆 CLI coverage 與 schema format，會比導入更多平台或資料庫更直接地提升可靠性。
